...
export default function InsightsIndex() {
  ...
  return (
    <>
      <PageHeader label={t.insights.label} title={p.insightsPage.title} intro={p.insightsPage.intro} />
          ...
          {ARTICLES.map((a, i) => (
            <Reveal as="li" key={a.slug} delay={i * 0.05}>
              <Link to={`/insights/${a.slug}`} data-cursor="hover"
                ...
                <span className="font-mono text-[11px] uppercase tracking-[0.14em] text-violet">{t.insights.items[i]?.tag ?? a.tag}</span>
                <span>
